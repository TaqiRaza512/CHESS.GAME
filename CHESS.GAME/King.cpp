#include "King.h"
King::King(Board* B, Position _P, COLOR _C) :Piece(B, _P, _C)
{
	//sym = ((_C == Black) ? 'K' : 'k');
...
bool King::IsLegalMove(Board* B, Position S, Position  D)
{
	...
	if (abs(S.ri-D.ri)==1 or abs(S.ci-D.ci)==1)
	{
...
void King::HighLight_The_Legal_Move(Board* B, Position S)
{
	Position D;
...
bool King::IsThereLegalMove(Board* B, Position S)
{
...
void King::Un_Highlight_The_Moves(Board* B, Position S, Position E)
{
	Position D;
	int colour;
	for (int ri = 0; ri < 8; ri++)
...
bool King::IsKing(Board* B)
{
	return true;

}
bool King::ISLEGAL(Board* B, Position S, COLOR C, Position  D)
{
